...
# ------------------------------------------------------------------------------------------------------------------------------------------------
# Configuration file: multicalibration.txt
...
def viccall(vars):
    ...
    global stations																	# number of stations
    global zones																	# number of zones
    global cellsize
    global zonematrix
    global ncols
    global nrows
    ...
    for line in lines:
        ...
        count_no+=1
    text_file.close()
    no_dv = no_of_var/zones

    with open('soil.txt','w') as my_csv:											# Modify soil file
        for sl in soilparam:
            if (sl[0]==1):															# Avoid add more row if the last row is blank
                count_no = 0
                x = int((sl[2]-cellsize/2-xcorner)/cellsize)
                y = nrows-int((sl[3]-cellsize/2-ycorner)/cellsize)-1
                if (zonematrix[x][y]>0):
                    for k in range(10):
                        if (VIC_vars[k]>0):
                            if (k==0):
                                sl[4] = vars[(zonematrix[x][y]-1)*no_dv+count_no]									#binfil
                                count_no+=1
                            elif (k==1):
                                sl[5] = vars[(zonematrix[x][y]-1)*no_dv+count_no]									#Ds
                                count_no+=1
                            elif (k==2):
                                sl[6] = vars[(zonematrix[x][y]-1)*no_dv+count_no]									#Dmax
                                count_no+=1
                            elif (k==3):
                                sl[7] = vars[(zonematrix[x][y]-1)*no_dv+count_no]									#Ws
                                count_no+=1
                            elif (k==4):
                                sl[8] = vars[(zonematrix[x][y]-1)*no_dv+count_no]									#c
                                count_no+=1
                            elif (k==5):
                                if (no_of_col==53): 
                                    sl[22] = vars[(zonematrix[x][y]-1)*no_dv+count_no]								#d1
                                else:
                                    sl[18] = vars[(zonematrix[x][y]-1)*no_dv+count_no]								#d1
                                count_no+=1
                            elif (k==6):
                                if (no_of_col==53): 
                                    sl[23] = vars[(zonematrix[x][y]-1)*no_dv+count_no]								#d2
                                else:
                                    sl[19] = vars[(zonematrix[x][y]-1)*no_dv+count_no]								#d2
                                count_no+=1
                            elif ((k==7) and (no_of_col==53)):
                                sl[24] = vars[(zonematrix[x][y]-1)*no_dv+count_no]									#d3 (in case the VIC model has more than 3 layers; modify this Python code accordingly)
    ...
    count_no = int(zonematrix[x][y]*no_dv)
    ...
    for sta in range(stations):
        os.chdir('../RoutingSetup')													# Modify when needed
        text_file = open('SITE'+str(sta+1)+'.csv','r')									# Modify when needed
        lines = text_file.read().split('\n')
        count_no = 0
        ...
        text_file = open('../Results/SITE'+str(sta+1)+'.day','r')									# Modify when needed
        lines = text_file.read().split('\n')
        count_no = 0
    ...
    meanmsde/=stations
    meanroce/=stations
    with open('progressNSE.txt','a') as f:
        f.write('\n'+str(meannse))
    with open('progressTRMSE.txt','a') as f:
        f.write('\n'+str(meantrmse))
#   Normalized NSE
    if meannse<=0:
        standnash = 1
        ...
        standnash = 1 - meannse
#   Normalized TRMSE
    standtrmse = meantrmse/pow(nortrmse/(number_of_days-spinning_period),0.5)
    if (standtrmse>1):
        standtrmse = 1
    elif (standtrmse<0):
        standtrmse = 0
#   Normalized MSDE
    standmsde = meanmsde / normsde * (number_of_days-spinning_period)
    if (standmsde>1):
        standmsde = 1
    elif (standmsde<0):
        standmsde = 0
...
os.chdir('../../RoutingSetup')
text_file = open('multicalibration.txt','r')
lines = text_file.read().split('\n')
...
text_file.close()
text_file = open('flowdirection.txt','r')
lines1 = text_file.read().split('\n')
ncols  = int(lines1[0].split('\t')[1])
nrows  = int(lines1[1].split('\t')[1])
xcorner = float(lines1[2].split('\t')[1])
ycorner = float(lines1[3].split('\t')[1])
cellsize = float(lines1[4].split('\t')[1])
text_file.close()
with open('progressNSE.txt','w') as f:
    f.write('Start ....')
with open('progressTRMSE.txt','w') as f:
    f.write('Start ....')
text_file = open('zonematrix.txt','r')
lines2 = text_file.read().split('\n')
zonematrix = [[0 for x in range(ncols)] for y in range(nrows)]
i = 0
for line in lines2:
    for j in range(ncols):
        zonematrix[i][j] = int(line.split('\t')[j])
    i+=1
text_file.close()
...
no_vars = no_of_var
if (int(lines[16].split('\t')[0])>0):
    VIC_types.append(Real(0.5, 5))																#Velocity
    no_vars+=1
if (int(lines[17].split('\t')[0])>0):
